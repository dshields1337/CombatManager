...
- Aligned every Sequence HP stepper to fixed right-side columns. Current HP is right-aligned in a fixed field and maximum HP reserves fixed width, so one-, two-, and three-digit values share the same units position without moving either button.
- Debug Android build succeeded with 0 warnings and 0 errors; API 36 verification confirmed identical control bounds for values `1`, `24`, `84`, `103`, `147`, and `241`, with no clipping and a live process.
...
- Split Monsters into Bestiary and Custom Monsters sub-tabs. Custom templates persist Name, HP, AC, Touch AC, Flat-Footed AC, CMD, CMB, initiative modifier, and optional notes; they support creation, editing/renaming, deletion, details, and direct addition to encounters.
- Custom-monster encounter copies behave as monsters rather than party members, retain quick combat stats, notes, mini labels, conditions, duplication, initiative, HP, active autosave, and named encounter snapshot compatibility independently of later template edits or deletion.
- Added regression coverage for custom-monster library validation/persistence and combat-roster round trips. API 36 verification created and renamed `Brass Sentinel`, added it to the encounter, displayed AC 21 / FF 19 / Touch 11 / CMD 28 / CMB +16, restored both library and encounter copy after force-stop, then deleted the template while retaining its encounter copy.
- Combat/Monsters rows now display each participant's initiative modifier before rolling and show both total and modifier afterward. Added explicit regression coverage confirming custom monsters use their saved modifier in one-tap Sequence initiative.
- Custom monsters added after combat has started now immediately roll a natural d20 plus their saved modifier, persist both natural and total initiative, enter the ordered Sequence, and preserve the current active turn.
- Existing active encounters are repaired once on launch by rolling any custom monster created before this fix that still lacks initiative, so its bracketed natural roll and total appear without rebuilding the encounter.
- Added checkbox multi-selection to Custom Monsters, matching the Bestiary workflow. Any number of reusable templates can be selected and copied into the encounter in one operation; mid-combat additions each receive a natural initiative roll plus their saved modifier.
- Sequence rows now show conditions with their remaining round count directly above the HP editor, remove them automatically at zero through the existing turn lifecycle, and replace them with `DOWN / DEAD` at zero HP. The active combatant now uses a bold outline instead of a blue fill, preserving monster health-status colours.
- Extended the Sequence health-status colours to active party characters using the same 51%/20%/zero boundaries as monsters, while inactive party members remain greyed out. Restored the arrow marker for the current turn alongside the bold outline.
